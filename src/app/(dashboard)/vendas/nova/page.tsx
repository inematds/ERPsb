...
import { ArrowLeft, Plus, Minus, Trash2, Search, ShoppingCart } from 'lucide-react';
...
export default function NovaVendaPage() {
  ...
  useEffect(() => {
    async function loadPaymentMethods() {
      const res = await fetch('/api/v1/formas-pagamento?active=true');
      if (res.ok) {
        const json = await res.json();
        setPaymentMethods(json.data);
        const defaultMethod = json.data.find((m: FormaPagamento) => m.isDefault);
  ...
  const handleConfirm = async () => {
    ...
    try {
      // Create the sale
      const createRes = await fetch('/api/v1/vendas', {
        method: 'POST',
      ...
      if (!createRes.ok) {
        ...
        toast.error(json.error || 'Erro ao criar venda');
        return;
      }

      const { data: venda } = await createRes.json();

      // Confirm the sale immediately
      const confirmRes = await fetch(`/api/v1/vendas/${venda.id}/confirmar`, {
        method: 'POST',
      });
      ...
      if (confirmRes.ok) {
        ...
        toast.error(json.error || 'Erro ao confirmar venda');
      }
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="p-4 space-y-4">
        ...
        <CardContent className="py-3">
          ...
          <div className="flex flex-wrap gap-2">
            {paymentMethods.map((pm) => (
              <Button
                key={pm.id}
                variant={selectedPaymentId === pm.id ? 'default' : 'outline'}
                size="sm"
                onClick={() => setSelectedPaymentId(pm.id)}
              >
                {PAYMENT_ICONS[pm.type] || pm.name}
              </Button>
            ))}
          </div>
      ...
      <Button
        ...
        disabled={saving || items.length === 0 || !selectedPaymentId}
      >
        <ShoppingCart className="h-5 w-5 mr-2" />
        {saving ? 'Confirmando...' : `Confirmar Venda${total > 0 ? ` - ${formatCurrency(total)}` : ''}`}
      </Button>
    </div>
  );
}
